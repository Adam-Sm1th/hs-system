<template>
  <el-card style="box-sizing: border-box;min-height: 95%; margin: 10px; min-height: 95%;">
    ...
    <el-table :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" v-loading="loading"
      ...
      <el-table-column label="医护ID" width="200" align="center">
      ...
      <el-table-column fixed="right" label="审核操作" min-width="120" align="center">
        <template #default="scope">
          <div style="white-space: nowrap;">
            <el-button size="small" type="danger" @click="handleCshiftDelay(scope.row)" :icon="CloseBold" circle
              style="margin-left: 15px;" :disabled="!(auditShow && scope.row.cshiftStatus == 1)">
            </el-button>
            <el-button size="small" type="success" @click="handleCshiftChange(scope.row)" :icon="Star" circle
              style="margin-left: 15px;" :disabled="!(auditShow && scope.row.cshiftStatus == 1)">
            </el-button>
          </div>
        </template>
...
import { Avatar, Edit, Delete, Select, CloseBold, Star } from '@element-plus/icons-vue'
...
const handleReset = () => {
  ...
  searchForm.value.cshiftStatus = 1;

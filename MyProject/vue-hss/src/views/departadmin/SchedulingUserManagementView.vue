        ...
        <el-calendar ref="calendar" v-loading="calendarLoading">
          <template #header="{ date }">
            <div style="display: flex; align-items: center;">
              <el-select placeholder="选择医护人员" style="width: 150px" size="big" v-model="searchForm.userId"
              ...
              <span style="margin-left: 10px;">积极性：{{ allUser.find(item => item.userId ===
                searchForm.userId)?.userProactivity }}</span>
            </div>
        ...
        <el-card style="max-width: 480px">
          ...
          <el-scrollbar height="200px">
            <el-table :data="rightListData" style="width: 100%" v-loading="alreadyLoading" stripe>
              ...
              <el-table-column fixed="right" label="取消" :width="60" align="center" v-if="operatorShow">
                <template #default="scope">
                  <div style="white-space: nowrap;">
                    <el-button size="small" type="warning" @click="cancelSche(scope.row)" :icon="Minus" circle
          ...
          <template #footer>
            <span>可排班</span>
            <el-scrollbar height="200px">
              <el-table :data="enableListData" style="width: 100%" v-loading="weiLoading" stripe>
                <el-table-column align="center" label="排班名称" show-overflow-tooltip>
                  <template #default="scope">
                    <span style="
                  cursor: pointer;
                ...
                <el-table-column fixed="right" label="排班" :width="60" align="center" v-if="operatorShow">
                  <template #default="scope">
                    <div style="white-space: nowrap;">
                      <el-button size="small" type="primary" @click="addUserSche(scope.row)" :icon="Plus" circle
                        :disabled="!(scope.row.scheStatus === 1 || scope.row.scheStatus === 2)">
                      </el-button>
                    </div>
                  </template>
...
import { Avatar, Edit, Delete, Select, CloseBold, Plus, Minus } from '@element-plus/icons-vue'
...
onMounted(async () => {
  ...
  searchForm.value.userId = allUser.value[0].userId;
...
const cancelSche = async (data: ScheInterface.SchedulingVO) => {
  const cancelData = ref<ScheInterface.SchedulingUser>({
    scheId: data.scheId ?? 0,
    userId: data.userId ?? ""
  });
  await ScheRequest.scheUserDelService(cancelData.value)
  reloadAllTable();
}

const addUserSche = async (data: ScheInterface.SchedulingVO) => {
  const cancelData = ref<ScheInterface.SchedulingUser>({
    scheId: data.scheId ?? 0,
    userId: searchForm.value.userId ?? ""
  });
  await ScheRequest.scheUserAddService(cancelData.value)
  reloadAllTable();
}

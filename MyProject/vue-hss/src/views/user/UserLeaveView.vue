      ...
      <el-table-column fixed="right" label="撤回" min-width="120" align="center">
        <template #default="scope">
          <div style="white-space: nowrap;">
            <el-button size="small" type="danger" @click="handleLeaveDelete(scope.row)" :icon="Back" circle
              :disabled="!(auditShow && scope.row.leaveStatus == 1)">
            </el-button>
          </div>
        </template>
      </el-table-column>
...
import { Avatar, Edit, Delete, Select, CloseBold, Back } from '@element-plus/icons-vue'
...
const handleReset = () => {
  ...
  searchForm.value.leaveStatus = null;
  reloadtableData()
  //处理搜索框恢复
};


const handleLeaveDelete = async (row: LeaveInterface.LeaveFormInterface) => {
  console.log(row.leaveId)
  const t = await LeaveRequest.leaveDeleteService(row.leaveId);
